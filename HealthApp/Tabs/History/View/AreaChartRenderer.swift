...
@objc open class AreaChartFormatter: NSObject, FillFormatter {
    ...
    public func getBoundaryDataSet() -> LineChartDataSet? { return boundaryDataSet }
}

open class CustomLineChartRenderer: LineChartRenderer {
    override open func drawLinearFill(context: CGContext, dataSet: LineChartDataSetProtocol, trans: Transformer,
                                      bounds: XBounds) {
        guard let dataProvider = dataProvider,
              let areaChartFormatter = dataSet.fillFormatter as? AreaChartFormatter,
              let boundarySet = areaChartFormatter.getBoundaryDataSet() else {
            return
        }

        let filled = generateFilledPath(dataSet: dataSet,
                                        fillMin: dataSet.fillFormatter?.getFillLinePosition(
                                            dataSet: dataSet, dataProvider: dataProvider) ?? 0.0,
                                        boundaryDataSet: boundarySet,
    ...
    fileprivate func generateFilledPath(dataSet: LineChartDataSetProtocol, fillMin: CGFloat,
        ...
        if let entry = dataSet.entryForIndex(bounds.min) {
            if let boundaryEntry = boundaryDataSet.entryForIndex(bounds.min) {
                filled.move(to: CGPoint(x: CGFloat(entry.x), y: CGFloat(boundaryEntry.y * phaseY)), transform: matrix)
            } else {
                filled.move(to: CGPoint(x: CGFloat(entry.x), y: fillMin), transform: matrix)
            }
            filled.addLine(to: CGPoint(x: CGFloat(entry.x), y: CGFloat(entry.y * phaseY)), transform: matrix)
        }

        for xVal in stride(from: (bounds.min + 1), through: bounds.range + bounds.min, by: 1) {
            guard let entryCurr = dataSet.entryForIndex(xVal) else {
                continue
            }

            if isDrawSteppedEnabled {
                guard let entryPrev = dataSet.entryForIndex(xVal - 1) else {
                    continue
                }
                filled.addLine(to: CGPoint(x: CGFloat(entryCurr.x), y: CGFloat(entryPrev.y * phaseY)),
                               transform: matrix)
            }
            filled.addLine(to: CGPoint(x: CGFloat(entryCurr.x), y: CGFloat(entryCurr.y * phaseY)), transform: matrix)
        }
        ...
        if let entry = dataSet.entryForIndex(bounds.range + bounds.min) {
            if let boundaryEntry = boundaryDataSet.entryForIndex(bounds.range + bounds.min) {
                filled.addLine(to: CGPoint(x: CGFloat(entry.x), y: CGFloat(boundaryEntry.y * phaseY)),
                               transform: matrix)
            } else {
        ...
        for xVal in stride(from: (bounds.min + 1), through: bounds.range + bounds.min, by: 1).reversed() {
            guard let entryCurr = boundaryDataSet.entryForIndex(xVal) else {
                continue
            }

            if isDrawSteppedEnabled {
                guard let entryPrev = boundaryDataSet.entryForIndex(xVal - 1) else {
                    continue
                }
                filled.addLine(to: CGPoint(x: CGFloat(entryCurr.x), y: CGFloat(entryPrev.y * phaseY)),
                               transform: matrix)
